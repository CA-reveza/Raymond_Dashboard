    ...
    <div className="space-y-6 animate-in fade-in duration-200">
        ...
        {coupons.map((c) => {
          ...
          return (
            <Card key={c.id} className="relative overflow-hidden flex flex-col justify-between">
              <div>
                <div className="flex items-start justify-between gap-2 mb-3">
                  <div className="flex items-center gap-2">
                    <div className="p-2 bg-[#122B52]/10 text-[#122B52] rounded-xl font-bold">
                      <Ticket className="w-5 h-5" />
                    </div>
                    <div>
                      <div className="text-base font-extrabold text-slate-900 dark:text-slate-100 font-mono tracking-wider">
                        {c.code}
                      </div>
                ...
                <div className="grid grid-cols-2 gap-2 text-xs bg-slate-50 dark:bg-white/5 p-3 rounded-xl border border-slate-100 dark:border-white/10 mb-4">
                  <div>
                    <span className="text-slate-400 dark:text-slate-500 font-medium">Discount Value:</span>
                    <div className="font-extrabold text-[#122B52] text-sm mt-0.5">
                      {c.discountType === 'Percentage' ? `${c.discountValue}% OFF` : `₹${c.discountValue} FLAT`}
                    </div>
                  </div>
                  <div>
                    <span className="text-slate-400 dark:text-slate-500 font-medium">Min Order Value:</span>
                ...
                <div className="space-y-1 text-xs mb-4">
                  ...
                  <div className="w-full bg-slate-100 dark:bg-white/10 rounded-full h-2 overflow-hidden">
                    <div
                      className="bg-gradient-to-r from-amber-500 to-[#122B52] h-2 rounded-full"
                ...
                <div className="bg-rose-50/50 border border-rose-100 rounded-xl p-2.5 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <div className="p-1.5 bg-[#122B52]/10 rounded-lg text-[#122B52]">
              ...
              <div className="pt-3 border-t border-slate-100 dark:border-white/10 flex items-center justify-between text-xs mt-4">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1 text-xs font-semibold text-slate-700 dark:text-slate-300 hover:text-[#122B52] cursor-pointer"
                >
                  {copiedCode === c.code ? (
                    <>
                      <Check className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" /> Copied Code!
                    </>
                  ) : (
                    <>
        ...
        footer={
          <div className="flex items-center justify-between w-full text-xs">
            <span className="text-slate-500 dark:text-slate-400">
              ...
              <strong className="text-[#122B52] font-mono text-sm">
      ...
      >
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 bg-slate-50 dark:bg-white/5 p-3 rounded-xl border border-slate-200 dark:border-white/10 text-xs">
            <div>
              <span className="text-slate-400 dark:text-slate-500 font-medium block">Coupon Offer:</span>
              <span className="font-extrabold text-[#122B52] text-sm">
            ...
            <div>
              <span className="text-slate-400 dark:text-slate-500 font-medium block">Redemption Count:</span>
              <span className="font-bold text-slate-800 dark:text-slate-200 text-sm">
                {activeRedemptions.length} Customer{activeRedemptions.length !== 1 ? 's' : ''}
              </span>
            </div>
            <div>
              <span className="text-slate-400 dark:text-slate-500 font-medium block">Applicable Scope:</span>
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 bg-white dark:bg-[#141F38] text-xs border border-slate-300 rounded-xl focus:border-[#122B52] outline-none"
            />
          </div>
          ...
          <div className="overflow-x-auto border border-slate-200 dark:border-white/10 rounded-xl">
            <table className="w-full text-left text-xs">
              <thead className="bg-slate-50 dark:bg-white/5 border-b border-slate-200 dark:border-white/10 text-[11px] text-slate-500 dark:text-slate-400 uppercase tracking-wider font-semibold">
                <tr>
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-white/10 bg-white dark:bg-[#141F38]">
                {filteredRedemptions.length > 0 ? (
                  filteredRedemptions.map((r) => (
                    <tr key={r.id} className="hover:bg-slate-50/80 transition-colors">
                      <td className="px-3.5 py-3">
                        <div className="font-bold text-slate-900 dark:text-slate-100">{r.customerName}</div>
                        <div className="text-[11px] text-slate-400 dark:text-slate-500">{r.customerEmail}</div>
                        <div className="text-[10px] text-slate-400 dark:text-slate-500 font-mono">{r.customerPhone}</div>
                      </td>
                      <td className="px-3.5 py-3">
                        <LoyaltyBadge tier={r.loyaltyTier} />
                      </td>
                      <td className="px-3.5 py-3 font-mono">
                        <div className="font-bold text-slate-800 dark:text-slate-200">{r.orderId}</div>
                        <div className="text-[10px] text-slate-400 dark:text-slate-500">{r.redeemedAt}</div>
                      ...
                      <td className="px-3.5 py-3 text-right">
                        <div className="font-extrabold text-[#122B52] font-mono text-xs">
        ...
        <div className="space-y-4 text-xs">
          <div>
            ...
            <input
              ...
              className="w-full font-mono uppercase font-bold text-sm bg-white dark:bg-[#141F38] p-2.5 rounded-xl border border-slate-300 focus:border-[#122B52] outline-none"
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              className="w-full text-xs bg-white dark:bg-[#141F38] p-2.5 rounded-xl border border-slate-300 focus:border-[#122B52] outline-none"
            />
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                className="w-full font-bold bg-white dark:bg-[#141F38] p-2.5 rounded-xl border border-slate-300 focus:border-[#122B52] outline-none"
              />
            </div>
            <div>
              <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">
