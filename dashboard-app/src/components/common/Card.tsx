...
export const Card: React.FC<CardProps> = ({
  ...
  return (
    <div
      onClick={onClick}
      className={`bg-white dark:bg-slate-800 rounded-[18px] border border-gray-100 dark:border-slate-700 p-6 shadow-xs ${
...
export const MetricCard: React.FC<MetricCardProps> = ({
  ...
  subtext,
  icon,
  iconBgColor = 'bg-rose-50 text-[#2D74B2]',
}) => {
  return (
    <Card hoverEffect={true}>
      ...
      {subtext && <p className="text-[11px] text-gray-400 dark:text-slate-500 mt-1 font-medium">{subtext}</p>}
    </Card>
  );
};
